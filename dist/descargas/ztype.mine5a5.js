  ...
  ig.Sound = ig.Class.extend({
    ...
    _fadeStep: function () {
      var v =
        this.fadeTimer
          .delta()
          .map(-this.fadeTimer.target, 0, 0, 1)
          .limit(0, 1) * this.volume;
      if (v <= 0.01) {
        this.stop();
      ...
      update: function () {
        ...
        for (var i = 0; i < this.items.length; i++) {
          ...
          if (
            ...
            ig.input.mouse.y > (ys - 10) * this.scale &&
            ig.input.mouse.y < (ys + this.font.height * 1.2) * this.scale
          ) {
            hoverItem = item;
            this.current = i;
          }
...
ig.baked = true;
ig.module("game.menus.interstitial")
  .requires("game.menus.base")
  .defines(function () {
    ...
    MenuInterstitial = Menu.extend({
      ...
      update: function () {
        ...
        if (
          ig.input.mouse.x > bx &&
          ig.input.mouse.x < bx + bw &&
          ig.input.mouse.y > by &&
          ig.input.mouse.y < by + bh
        ) {
          ig.system.canvas.style.cursor = "pointer";
        }
      },
      onclick: function (event) {
        ...
        if (x > bx && x < bx + bw && y > by && y < by + bh) {
          window.open(
            "http://nofatenetmusic.bandcamp.com/album/the-phoboslab-works"
          );
        }
      },
      draw: function () {
...
ig.baked = true;
ig.module("game.menus.stats")
  .requires("game.menus.base", "game.xhr")
  .defines(function () {
    StatsView = ig.Class.extend({
      ...
      load: function () {
        if (window.Cocoon || window.Ejecta) {
          ...
          ig.xhr("/api/", { load: true }, this.receivedData.bind(this));
        }
      },
      ...
      drawLoading: function (x, y) {
        ...
        var indicator =
          ...
          ((this.timer.delta() * 4) | 0) % this.workingIndicator.length
          ];
      ...
      drawGraph: function (x, y) {
        ...
        if (this.isExcerpt) {
          ...
          ctx.globalAlpha = 0.5;
          this.font.draw(
            "(trend over " + this.totalGames + " games)",
            x / scale,
            (y + this.height) / scale,
            ig.Font.ALIGN.LEFT
          );
        ...
        for (var i = 0; i < numLabels; i++) {
          ...
          this.font.draw(
            label,
            xp / scale,
            (yp - (this.font.height * scale) / 2) / scale,
            ig.Font.ALIGN.RIGHT
          );
          ctx.restore();
          ctx.beginPath();
        ...
        var lastp = null;
        var max = Math.min(
          this.data.length * this.timer.delta(),
          this.data.length
        );
        for (var i = 0; i < max; i++) {
          var d = this.data[i];
          ...
          var rx = x + (rw - 3) * v,
            ry =
              y + this.height - (this.height - 3) * (d.score / this.scoreScale);
...
ig.baked = true;
ig.module("game.menus.game-over")
    ...
    MenuGameOver = Menu.extend({
      ...
      init: function () {
        ...
        this.stats.submit({
          ...
          accuracy: ig.game.hits
            ? (ig.game.hits / (ig.game.hits + ig.game.misses)) * 100
            : 0,
        });
        this.timer = new ig.Timer();
      },
      ...
      draw: function () {
        ...
        this.fontTitle.draw(
          "WAVE " + ig.game.wave.wave.zeroFill(3),
          252,
          ys + 25
        );
        ig.system.context.drawImage(
          this.separatorBar.data,
          24,
          ys + 70,
          432,
          2
...
ig.baked = true;
ig.module("game.menus.detailed-stats")
  .requires("game.menus.base", "game.xhr")
  .defines(function () {
    DetailedStats = ig.Class.extend({
      ...
      show: function () {
        ...
        setTimeout(function () {
          ...
          setTimeout(function () {
            ...
            ig.xhr("/api/", { loadAll: true }, that._initChart.bind(that));
          }, 400);
        }, 16);
        ...
        ig.$("#ztype-stats-share-facebook").addEventListener(
          "click",
          this.shareFacebookBound
        );
        this.shareTwitterBound = this.shareTwitter.bind(this);
        ig.$("#ztype-stats-share-twitter").addEventListener(
          "click",
          this.shareTwitterBound
        );
      },
      ...
      close: function () {
        ...
        ig.$("#ztype-stats-share-facebook").removeEventListener(
          "click",
          this.shareFacebookBound
        );
        ig.$("#ztype-stats-share-twitter").removeEventListener(
          "click",
          this.shareTwitterBound
        );
        ...
        for (var i = 0; i < this.data.length; i++) {
          ...
          if (mx < rx + md && mx > rx - md && my < ry + md && my > ry - md) {
            ...
            if (!plot || plot.dist > dist) {
              plot = { x: rx, y: ry, game: g, dist: dist };
            }
          }
        }
        if (plot) {
      ...
      saveScoresScreenshot: function (callback) {
        ...
        ig.xhr("/api/", { saveScreenshot: true, data: png }, callback);
      },
      shareFacebook: function () {
        ...
        this.saveScoresScreenshot(function (response) {
          ...
          shareWindow.location.href =
            ...
            "?app_id=112268002182064" +
            "&display=popup" +
            "&caption=" +
            encodeURIComponent("My ZTYPE Scores") +
            "&link=" +
            encodeURIComponent("http://zty.pe/") +
            "&picture=" +
      ...
      shareTwitter: function () {
        var shareWindow = window.open(
          "about:blank",
          "facebook-share",
          "toolbar=0,status=0,width=575,height=400"
        );
        this.saveScoresScreenshot(function (response) {
          var imageUrl = "http://zty.pe/" + response.file;
          shareWindow.location.href =
            ...
            "?text=" +
            encodeURIComponent("My #ZTYPE Scores: " + imageUrl + " - ");
        });
      },
    });
  });
...
ig.module("game.ease").defines(function () {
  ig.ease = {
    ...
    inOutBounce: function (t, b, c, d) {
      if (t < d / 2) return ig.ease.inBounce(t * 2, 0, c, d) * 0.5 + b;
      return ig.ease.outBounce(t * 2 - d, 0, c, d) * 0.5 + c * 0.5 + b;
    },
  };
});
...
ig.baked = true;
ig.module("game.menus.title")
  .requires(
    "game.menus.base",
    "game.menus.settings",
    "game.menus.detailed-stats",
    "game.ease"
  )
  .defines(function () {
    ...
    MenuItemAppStore = MenuItem.extend({
      ...
      ok: function () {
        window.location.href = ig.ua.iOS
          ? "https://goo.gl/qSntlx"
          : "https://goo.gl/cg7rmI";
      },
    });
    MenuTitle = Menu.extend({
      ...
      init: function () {
        ...
        if (window.Cocoon || window.Ejecta) {
        } else if (ig.ua.iOS || ig.ua.android) {
      ...
      draw: function () {
        ...
        ctx.globalAlpha =
          this.alpha +
          Math.sin(this.transition * Math.PI) * (1 - Math.random() * 0.5);
        ctx.save();
        ctx.scale(this.scale, this.scale);
        this.scroll += ig.system.tick * 10;
        this.scroll = this.scroll % this.background.height;
        ...
        this.background.draw(0, this.scroll);
        this.phoboslab.draw(
          (ig.system.width / this.scale - this.phoboslab.width) / 2,
          80 - ig.ease.inOutQuad(this.transition, 0, 2800, 1)
        );
        this.ztype.draw(
          (ig.system.width / this.scale - this.ztype.width) / 2,
          150 - ig.ease.inOutQuad(this.transition, 0, 1000, 1)
        );
        ...
        if (this.items.length > 1) {
          this.items[1].y = ig.ease.inOutQuad(
            this.transition,
            ig.system.height / this.scale - 200,
            1900,
            1
          );
        }
        if (this.items.length > 2) {
          this.items[2].y = ig.ease.inOutQuad(
            this.transition,
            ig.system.height / this.scale - 140,
            2100,
            1
          );
        }
        if (this.items.length > 3) {
          this.items[3].y = ig.ease.inOutQuad(
            this.transition,
            ig.system.height / this.scale - 80,
            2200,
            1
          );
        }
        ...
        for (var i = 0; i < 10; i++) {
          ...
          ig.system.context.globalAlpha =
            (ey < 20 ? (ey / 20) * 0.7 : Math.max(0, 0.7 - ey / 400)) * 0.3;
          this.exhaust.draw(
            this.playerPos.x - 1 + Math.sin(ig.Timer.time + ey / 30) * 0.7,
            this.playerPos.y + ey
          );
        }
        ig.system.context.globalAlpha = 1;
        ...
        if (this.transition > 0.55 && this.transition < 0.62) {
          this.glitchGraph.draw(
            ig.system.width - this.glitchGraph.width - Math.random() * 10,
            10
          );
        }
        if (
          (this.transition > 0.6 && this.transition < 0.7) ||
          (this.transition > 0.3 && this.transition < 0.33)
        ) {
        ...
        if (this.transition > 0.8 && this.transition < 0.85) {
          this.glitchJPEG.draw(
            ig.system.width - this.glitchJPEG.width,
            300 + Math.random() * 100
          );
        }
        ctx.globalAlpha =
          Math.max(0.4 - this.transition, 0) +
          (this.infoIcon.hovered ? 0.3 : 0);
      ...
      update: function () {
        ...
        if (ig.input.mouse.y < 104 && ig.input.pressed("click")) {
          if (window.ejecta) {
            ejecta.openURL(
              "https://itunes.apple.com/us/artist/phoboslab/id312666931"
            );
          } else {
            window.open(
              "https://itunes.apple.com/us/artist/phoboslab/id312666931"
        ...
        this.parent();
        if (
          ig.input.mouse.x > ig.system.width - 64 &&
          ig.input.mouse.y > ig.system.height - 64
        ) {
          if (ig.input.pressed("click")) {
...
ig.baked = true;
ig.module("game.avenir-next")
  .requires("impact.font")
  .defines(function () {
    var METRICS = [
      0, 0, 1, 1, -29, -21, 5, 2, 0, 4, 15, 1, 2, 8, 7, 0, 6, 5, 3, 2, 11, 14,
      0, 12, 15, 0, 2, 11, 27, 0, 11, 19, 0, 0, 11, 39, 0, 17, 17, 0, 1, 17, 57,
      0, 14, 15, 1, 2, 15, 72, 0, 3, 5, 1, 2, 5, 76, 0, 6, 19, 0, 1, 5, 83, 0,
      5, 19, 0, 1, 5, 89, 0, 10, 8, 0, 2, 9, 100, 0, 12, 11, 0, 5, 14, 113, 0,
      5, 5, 0, 15, 5, 119, 0, 7, 2, 0, 11, 7, 127, 0, 4, 2, 1, 15, 5, 132, 0,
      10, 17, -1, 1, 8, 143, 0, 11, 15, 0, 2, 11, 155, 0, 7, 15, 1, 2, 11, 163,
      0, 11, 15, 0, 2, 11, 175, 0, 11, 15, 0, 2, 11, 187, 0, 11, 15, 0, 2, 11,
      199, 0, 11, 15, 0, 2, 11, 211, 0, 11, 15, 0, 2, 11, 223, 0, 10, 15, 0, 2,
      11, 234, 0, 11, 15, 0, 2, 11, 246, 0, 11, 15, 0, 2, 11, 258, 0, 4, 10, 1,
      7, 5, 263, 0, 5, 13, 0, 7, 5, 269, 0, 12, 11, 0, 5, 14, 282, 0, 12, 4, 0,
      177, 66, 5, 19, 1, -2, 5, 183, 66, 8, 19, -1, -2, 5, 192, 66, 8, 19, -1,
      -2, 5, 201, 66, 17, 15, -1, 2, 16, 219, 66, 14, 19, 0, -2, 16, 234, 66,
      18, 19, 0, -2, 18, 253, 66, 18, 19, 0, -2, 18, 272, 66, 18, 19, 0, -2, 18,
      291, 66, 18, 19, 0, -2, 18, 310, 66, 18, 19, 0, -2, 18, 329, 66, 12, 10,
      0, 6, 14, 342, 66, 18, 15, 0, 2, 18, 361, 66, 13, 19, 0, -2, 14, 375, 66,
      13, 19, 0, -2, 14, 389, 66, 13, 19, 0, -2, 14, 403, 66, 13, 19, 0, -2, 14,
      417, 66, 14, 19, -1, -2, 12, 432, 66, 11, 15, 0, 2, 12, 444, 66, 11, 16,
      0, 1, 12, 456, 66, 10, 15, 0, 2, 11, 467, 66, 10, 15, 0, 2, 11, 478, 66,
      10, 15, 0, 2, 11, 489, 66, 10, 15, 0, 2, 11, 500, 66, 10, 15, 0, 2, 11, 0,
      88, 10, 16, 0, 1, 11, 11, 88, 18, 10, 0, 7, 18, 30, 88, 11, 15, 0, 7, 10,
      42, 88, 12, 15, 0, 2, 12, 55, 88, 12, 15, 0, 2, 12, 68, 88, 12, 15, 0, 2,
      12, 81, 88, 12, 15, 0, 2, 12, 94, 88, 6, 15, -1, 2, 5, 101, 88, 5, 15, 1,
      2, 5, 107, 88, 8, 15, -1, 2, 5, 116, 88, 8, 15, -1, 2, 5, 125, 88, 12, 16,
      0, 1, 12, 138, 88, 10, 15, 0, 2, 12, 149, 88, 12, 15, 0, 2, 12, 162, 88,
      12, 15, 0, 2, 12, 175, 88, 12, 15, 0, 2, 12, 188, 88, 12, 15, 0, 2, 12,
      201, 88, 12, 15, 0, 2, 12, 214, 88, 12, 9, 0, 6, 14, 227, 88, 13, 12, 0,
      6, 12, 241, 88, 10, 15, 0, 2, 12, 252, 88, 10, 15, 0, 2, 12, 263, 88, 10,
      15, 0, 2, 12, 274, 88, 10, 15, 0, 2, 12, 285, 88, 11, 20, 0, 2, 10, 297,
      88, 12, 21, 1, 1, 13,
    ];
    var METRIC_OFFSETS = {
      SX: 0,
    ...
    ig.FontAvenirNext = ig.Font.extend({
      ...
      _widthForLine: function (s) {
        ...
        for (var i = 0; i < s.length; i++) {
          var advanceIdx =
            (s.charCodeAt(i) - this.firstChar) * O.ENTRIES_PER_GLYPH +
            O.ADVANCE;
      ...
      _drawChar: function (c, targetX, targetY) {
        ...
        ig.system.context.drawImage(
          ...
          sw,
          sh,
          ig.system.getDrawPos(targetX + offsetX),
          ig.system.getDrawPos(targetY + offsetY),
          sw,
          sh
        );
  ...
  .defines(function () {
    EntityParticle = ig.Entity.extend({
      ...
      update: function () {
        ...
        this.currentAnim.alpha = this.idleTimer
          .delta()
...
ig.module("game.entities.explosion")
  .requires("impact.entity", "impact.entity-pool", "game.entities.particle")
  .defines(function () {
    ...
    window.EntityExplosionHugeGlitch = EntityExplosionHuge.extend({
      ...
      draw: function () {
        ...
        if (d < 0.1 || (d > 0.2 && d < 0.3)) {
          this.glitchJPEG.draw(
            this.pos.x - 240 + (200 - Math.random() * 400),
    ...
    EntityEnemy = ig.Entity.extend({
      ...
      init: function (x, y, settings) {
        ...
        var length = Math.random()
          .map(0, 1, this.wordLength.min, this.wordLength.max)
          .round();
        ...
        for (var i = 0; i < 20; i++) {
          if (l >= 2 && l <= 12) {
            ...
            w = String.fromCharCode(
              "a".charCodeAt(0) + (Math.random() * 26).floor()
            );
          }
          if (
            !ig.game.targets[w.charAt(0).toLowerCase()] ||
            !ig.game.targets[w.charAt(0).toLowerCase()].length
          ) {
            return w;
          }
        }
      ...
      draw: function () {
        ...
        if (this.reticleTimer) {
          ...
          var scale = (1 - d) * (1 - d) * 4;
          ctx.translate(
            this.pos.x + this.size.x / 2 - ig.game._rscreen.x,
            this.pos.y + this.size.y / 2 - ig.game._rscreen.y
          );
          ctx.scale(scale, scale);
      ...
      drawLabel: function () {
        ...
        var x =
          (this.pos.x - 6).limit(w + 2, ig.system.width - 1) +
          ig.game._rscreen.x;
        var y =
          (this.pos.y + this.size.y - 10).limit(2, ig.system.height - 19) +
          ig.game._rscreen.y;
      ...
      spawnExplosionParticles: function (killed) {
        var numParticles = killed
          ? this.numKillParticles
          : this.numHitParticles;
...
ig.baked = true;
ig.module("game.entities.enemy-missle")
...
ig.baked = true;
ig.module("game.entities.enemy-mine")
  .requires("game.entities.enemy")
  .defines(function () {
...
ig.baked = true;
ig.module("game.entities.enemy-destroyer")
  .requires("game.entities.enemy", "game.entities.explosion")
  .defines(function () {
    EntityEnemyDestroyer = EntityEnemy.extend({
      ...
      init: function (x, y, settings) {
        ...
        this.angle =
          ((Math.random().map(0, 1, 67, 90) +
            (this.pos.x > ig.system.width / 2 ? 22.5 : 0)) *
            Math.PI) /
          180;
        this.currentAnim.angle = this.angle - Math.PI / 2;
      },
...
ig.baked = true;
ig.module("game.entities.enemy-oppressor")
  .requires("game.entities.enemy")
  .defines(function () {
      ...
      update: function () {
        if (this.target) {
          ...
          if (
            ...
            currentDistance < this.target.size.y
          ) {
...
ig.module("game.keyboard")
  ...
  .defines(function () {
    ig.Keyboard = ig.Class.extend({
      ...
      hoverKey: null,
      expectedKeys: null,
      keyMetrics: { width: 64, height: 106, offsetTop: 32 },
      ...
      setLayout: function (layout) {
        ...
        for (var y = 0; y < this.layout.map.length; y++) {
          ...
          for (var x = 0; x < row.length; x++) {
            ...
            this.font.draw(
              key.toUpperCase(),
              (x + offset + 0.5) * this.keyMetrics.width,
              y * this.keyMetrics.height + this.keyMetrics.offsetTop,
              ig.Font.ALIGN.CENTER
            );
          }
        }
      ...
      getCurrentKey: function (touches) {
        ...
        var offsetX =
          (this.background.width -
            this.layout.map[row].length * this.keyMetrics.width) /
          2;
        ...
        if (this.expectedKeys) {
          ...
          for (var i = 0; i < this.expectedKeys.length; i++) {
            ...
            var keyX =
              kpos[0] * this.keyMetrics.width + this.keyMetrics.width / 2,
              keyY =
                kpos[1] * this.keyMetrics.height + this.keyMetrics.height / 2;
      ...
      drawFull: function () {
        ...
        if (this.hoverKey) {
          ...
          var alpha = this.unsetHoverKeyTimer
            ? this.unsetHoverKeyTimer.delta().map(0, 0.25, 1, 0)
            : 1;
          ctx.globalAlpha = 0.9 * alpha;
          hoverImage.draw(x, y);
          this.font.draw(
            this.hoverKey.toUpperCase(),
            x + 51,
            y + 24,
            ig.Font.ALIGN.CENTER
          );
      ...
      drawMultiplierBar: function (x, y) {
        ...
        var showMultiplierTime = this.multiplierTimer
          ? this.multiplierTimer.delta()
          : 0;
        if (showMultiplierTime < 0) {
          ig.system.context.globalAlpha = showMultiplierTime
            .map(-2, 0, 2, 0)
            .limit(0, 1);
          this.multiIndicator.drawTile(
            x + this.width * multiLength - 30,
            y - 22,
...
ig.baked = true;
ig.module("plugins.silent-loader")
...
ig.module("game.document-scanner").defines(function () {
  ...
  ig.DocumentScanner = ig.Class.extend({
    ...
    init: function (rootElement, enableNumbers, enablePunctionations) {
      this._reSkipNode =
        /^(script|style|head|link|meta|button|input|nav|label|img|svg|canvas|ins)$/i;
      this._reSkipClass =
        /head|mw-jump-link|foot|links|menu|nav|pagetop|subtext|side|notice|overlay|siteSub|infobox|hatnote|abstract|caption|disclaimer|byline/i;
      this._reTextNode =
        /^(em|strong|b|i|mark|cite|dfn|small|del|ins|sub|sup|a|#text|span|code)$/i;
      this._reHasWords = /\b[^\d\W]{2,}\b/g;
      this._reSplitSentences = /([.?!:\/]+[\s\[\]]+)(?=\w)/g;
      this._reHasEnglishWords =
        /\b(the|be|to|of|and|that|have|with|you|this|say)\b/i;
      if (enableNumbers) {
    ...
    traverse: function (element, elements) {
      ...
      var style =
        element instanceof HTMLElement
          ...
          : null;
      if (
        element.nodeName.match(this._reSkipNode) ||
        (element.id && element.id.match(this._reSkipClass)) ||
        (element.className && element.className.match(this._reSkipClass)) ||
        (style && (style.display == "hidden" || style.opacity === "0"))
      ) {
      ...
      for (var i = 0; i < element.childNodes.length; i++) {
        ...
        if (
          child.nodeName !== "A" &&
          (!child.nodeName.match(this._reTextNode) ||
            child.textContent.match(this._reHasWords))
        ) {
      ...
      if (this.childrenAreText(element)) {
        ...
        if (
          ...
          (element.nodeName === "#text" || element.offsetHeight > 0) &&
          tm &&
          tm.length > 0
        ) {
    ...
    splitIntoFragments: function (element, fragments, totalElements) {
      ...
      var text = element.textContent;
      var sentences = text
        .replace(this._reSplitSentences, "$1%%ZT%%")
        .split("%%ZT%%");
      ...
      for (var i = 0; i < sentences.length; i++) {
        ...
        spans.push(span);
        fragments.push({ text: sentences[i], element: span });
      }
    ...
    childrenAreText: function (element) {
      for (var i = 0; i < element.childNodes.length; i++) {
        if (
          !element.childNodes[i].nodeName.match(this._reTextNode) ||
          (element.id && element.id.match(this._reSkipClass)) ||
          (element.className && element.className.match(this._reSkipClass)) ||
          !this.childrenAreText(element.childNodes[i])
        ) {
          return false;
        }
      }
    ...
    highlightFragment: function (fragment, margin, dontDeHighlight) {
      ...
      if (this._currentFragment) {
        ...
        var target =
          ...
          this._currentFragment.getBoundingClientRect().top -
          (margin || this.highlightMarginTop);
    ...
    scrollTo: function (y) {
      ...
      if (!this._scrollAnimationId) {
        this._scrollAnimationId = setInterval(
          this._animateScroll.bind(this),
          5
        );
      }
    },
    ...
    nextAnimationStep: function (current) {
      ...
      this.$scanProgress.style.width =
        Math.min(1, current / this.fragments.length) * 100 + "%";
    },
...
ig.module("game.main")
  ...
  .defines(function () {
    ...
    ZType = ig.Game.extend({
      ...
      init: function () {
        ...
        if (window.Cocoon && window.Cocoon.Ad) {
          Cocoon.Ad.configure({
            android: { interstitial: "ca-app-pub-8533552145182353/1042008307" },
          });
        ...
        if (openFeedbackForm) {
          openFeedbackForm.addEventListener(
            "click",
            this.openFeedbackForm.bind(this)
          );
        }
      },
      reset: function () {
      ...
      nextDocFragment: function () {
        this.wave.fragment++;
        var fragment =
          ig.doc.fragments[(this.wave.fragment - 1) % ig.doc.fragments.length];
        if (!ig.ua.mobile) {
        ...
        var words = this.textHasPunctuations
          ? fragment.text.replace(/[’‘’’]/g, "'").split(this.reSplitNonWord)
          : fragment.text.replace(/['’‘’’]/g, "").split(this.reSplitNonWord);
        var filteredWords = [];
        ...
        var bigShipChance =
          (this.wave.types[0].count + this.wave.types[1].count) /
          this.wave.types[2].count;
        var wordLengthForBigShip =
          wordsByLength[Math.floor(wordsByLength.length * bigShipChance * 0.75)]
            .length;
        ...
        for (var i = 0; i < filteredWords.length; i++) {
          ...
          var type =
            w.length > wordLengthForBigShip
              ? Math.random() > 0.75
                ? EntityEnemyOppressor
                : EntityEnemyDestroyer
              : EntityEnemyMine;
          this.wave.spawn.push({ type: type, word: w, wait: wait });
        }
      },
      spawnCurrentWave: function () {
        if (!this.wave.spawn.length) {
          ...
          this.spawnEntity(
            ...
            x,
            y,
            { healthBoost: this.wave.healthBoost, word: spawn.word },
            true
          );
          this.wave.currentSpawnWait = spawn.wait
            ? this.wave.spawnWait * spawn.wait
            : this.wave.spawnWait;
        }
      },
        ...
        if (this.currentTarget) {
          ...
          if (hit) {
            ...
            if (target.dead) {
              ...
              var translated = this.translateUmlaut(
                this.prepareLetter(target.remainingWord.charAt(0))
              );
              if (this.keyboard) {
      ...
      setGameState: function () {
        ...
        var sx = ig.system.width / 2 - 6,
          sy =
            ig.system.height -
            this.keyboard.height * this.keyboard.drawScale -
            30;
      ...
      draw: function () {
        ...
        if (this.menu) {
          ...
          if (this.gameTransitionTimer) {
            ...
            this.menu.playerPos.y = ig.ease.inOutBack(
              dt,
              MenuTitle.prototype.playerPos.y,
              move,
              2
            );
      ...
      drawUI: function () {
        ...
        if (!ig.ua.mobile && this.idleTimer.delta() > 8) {
          ...
          ig.system.context.globalAlpha =
            (Math.sin(this.idleTimer.delta() * 4) * 0.25 + 0.75) * aa;
          this.font.draw(
    ...
    ZType.MULTIPLIER_TIERS = { 20: 2, 50: 3 };
    ZType.FLAG = {
      CASE_SENSITIVE: 1 << 0,
      PUNCTUATION: 1 << 1,
      NUMBERS: 1 << 2,
    ...
    if (window.ZTypeDocumentMode) {
      ...
      ig.doc = new ig.DocumentScanner(
        document.body,
        enableNumbers,
        enablePunctionations
      );
      if (!ig.ua.mobile) {
